...
int main(int argc, char **argv)
{
    ...
    ros::param::get("lidar_topic", liadr_topic); // Retrieve the topic name parameter
    if (argc < 3)
    {
        ROS_ERROR(
    ...
    else
    {
        ...
        {
            subRobosensePC = nh.subscribe(liadr_topic, 1, hesaiHandler_XYZI);
        }
        else if (std::strcmp("XYZIRT", argv[1]) == 0)
        {
            subRobosensePC = nh.subscribe(liadr_topic, 1, hesaiHandler_XYZIRT);
        }
        else
        {
            ROS_ERROR(argv[1]);
    ...
    ros::param::get("publish_lidar_topic", publish_lidar_topic); // Retrieve the topic name parameter
